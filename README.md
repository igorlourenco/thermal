...
Sources/
  ...
  tempsensors/
    ...
    HistoryStore.swift       rolling 24h samples: trends, peaks, chart data
    MenuBarApp.swift         SwiftUI MenuBarExtra app (model + views)
...
- Bare SwiftPM executable: no bundle yet, so no launch-at-login,
  notifications, or app icon until wrapped in an Xcode project.
- History is in-memory (per session); persistence comes with the app bundle.
